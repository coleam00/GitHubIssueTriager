...
export type IssueExportRow = IssueListRow & {
  github_created_at: string;
  github_updated_at: string | null;
  labels: string[];
};

export async function listIssues(filter: IssueFilter): Promise<IssueListRow[]> {
...
export async function listIssuesForExport(filter: IssueFilter): Promise<IssueExportRow[]> {
  const rows = (await sql`
    SELECT
      i.id,
      i.github_number,
