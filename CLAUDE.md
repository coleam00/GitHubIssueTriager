...
- `/api/classify-batch` streams NDJSON and runs up to 4 classifications in parallel (`CONCURRENCY` in its `route.ts`). Tune only if the model provider's rate limit genuinely allows it.
